...
$installReportScript = Join-Path $PSScriptRoot "new-install-test-report.ps1"
...
if (-not (Test-Path $installReportScript)) {
    throw "Missing helper: $installReportScript"
}
...
$installReportPath = & $installReportScript `
    -Mode $Mode `
    -VmType $VmType `
    -Firmware $Firmware `
...
$installReportPath = $installReportPath.ToString().Trim()

if (-not (Test-Path $installReportPath)) {
    throw "Install test report was not created: $installReportPath"
}

Write-Host "Prepared Lumina-OS installer test from GitHub Actions."
