...
$releaseEvidenceTargetAuditScript = Join-Path $PSScriptRoot "audit-release-evidence-target.ps1"
$releaseEvidenceRunbookScript = Join-Path $PSScriptRoot "new-release-evidence-runbook.ps1"
$releaseEvidenceActionPackScript = Join-Path $PSScriptRoot "new-release-evidence-action-pack.ps1"
...
if (-not (Test-Path $releaseEvidenceTargetAuditScript)) {
    throw "Missing smoke-test target: $releaseEvidenceTargetAuditScript"
}
...
if (-not (Test-Path $releaseEvidenceActionPackScript)) {
    throw "Missing smoke-test target: $releaseEvidenceActionPackScript"
}
...
if (-not (Test-Path $openNextReleaseActionScript)) {
    throw "Missing smoke-test target: $openNextReleaseActionScript"
}
...
if (-not (Test-Path $syncReleaseEvidenceSessionScript)) {
    throw "Missing smoke-test target: $syncReleaseEvidenceSessionScript"
}
...
if (-not (Test-Path $releaseValidationRunbookScript)) {
    throw "Missing smoke-test target: $releaseValidationRunbookScript"
}

if (-not (Test-Path $releaseValidationWorkboardScript)) {
    throw "Missing smoke-test target: $releaseValidationWorkboardScript"
}

if (-not (Test-Path $syncReleaseExecutionStatusScript)) {
    throw "Missing smoke-test target: $syncReleaseExecutionStatusScript"
}
...
if (-not (Test-Path $syncReleaseControlCenterScript)) {
    throw "Missing smoke-test target: $syncReleaseControlCenterScript"
}
...
try {
    ...
    $initialLoginAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseLoginTestReportPath -Target "login-test" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $initialInstallAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseInstallReportPath -Target "install" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $initialHardwareAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseHardwareReportPath -Target "hardware" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $initialChecklistTotal = [int]$initialLoginAudit.TotalChecklistItems + [int]$initialInstallAudit.TotalChecklistItems + [int]$initialHardwareAudit.TotalChecklistItems
    $initialChecklistSummary = "0/$initialChecklistTotal complete (0%)"

    Assert-Condition -Condition ($initialLoginAudit.TotalChecklistItems -gt 0 -and $initialLoginAudit.CheckedChecklistItems -eq 0) -Message "Initial login-test audit did not record an untouched checklist."
    Assert-Condition -Condition ($initialInstallAudit.TotalChecklistItems -gt 0 -and $initialInstallAudit.CheckedChecklistItems -eq 0) -Message "Initial install audit did not record an untouched checklist."
    Assert-Condition -Condition ($initialHardwareAudit.TotalChecklistItems -gt 0 -and $initialHardwareAudit.CheckedChecklistItems -eq 0) -Message "Initial hardware audit did not record an untouched checklist."
    Assert-Condition -Condition ($initialLoginAudit.ProgressState -eq "not-started") -Message "Initial login-test audit did not report not-started progress."
    ...
    Assert-Condition -Condition ($currentEvidencePackContent -match [regex]::Escape("- Evidence Ready Count: 0/3")) -Message "Current evidence pack summary did not record the initial ready count."
    Assert-Condition -Condition ($currentEvidencePackContent -match [regex]::Escape("- Evidence Checklist Progress: $initialChecklistSummary")) -Message "Current evidence pack summary did not record the initial checklist progress."
    ...
    $releaseEvidenceSessionPath = & $releaseEvidenceSessionScript `
        -EvidencePackPath $releaseEvidencePackPath `
        -ReleaseVersion "0.1.0-ci" `
        -RepoRoot $tempRoot `
        -OutputPathOnly

    Assert-Condition -Condition (Test-Path $releaseEvidenceSessionPath) -Message "Release evidence session was not created."
    $releaseEvidenceSessionContent = Get-Content -Raw $releaseEvidenceSessionPath
    Assert-Condition -Condition ($releaseEvidenceSessionContent -match [regex]::Escape("- Evidence Pack: $releaseEvidencePackPath")) -Message "Release evidence session did not record the evidence pack path."
    ...
    $releaseEvidenceActionPackPath = Get-MetadataValue -Content $releaseEvidenceSessionContent -Label "Action Pack Path"
    Assert-Condition -Condition (-not [string]::IsNullOrWhiteSpace($releaseEvidenceActionPackPath) -and (Test-Path $releaseEvidenceActionPackPath)) -Message "Release evidence session did not produce an evidence action pack."
    $releaseEvidenceActionPackDir = Split-Path -Parent $releaseEvidenceActionPackPath
    Assert-Condition -Condition (Test-Path (Join-Path $releaseEvidenceActionPackDir "00-run-next-step.ps1")) -Message "Release evidence action pack did not create the next-step launcher."
    Assert-Condition -Condition (Test-Path (Join-Path $releaseEvidenceActionPackDir "30-sync-evidence-session.ps1")) -Message "Release evidence action pack did not create the evidence-session sync helper."
    $initialNextEvidenceActionPath = Get-MetadataValue -Content $releaseEvidenceSessionContent -Label "Next Action Path"
    $initialNextEvidenceLauncherPath = Get-MetadataValue -Content $releaseEvidenceSessionContent -Label "Next Action Launcher"
    Assert-Condition -Condition ((Resolve-Path $initialNextEvidenceActionPath).Path -eq (Resolve-Path $releaseLoginTestReportPath).Path) -Message "Release evidence session did not resolve the initial next action to the login-test report."
    Assert-Condition -Condition ((Resolve-Path $initialNextEvidenceLauncherPath).Path -eq (Resolve-Path (Join-Path $releaseEvidenceActionPackDir "00-run-next-step.ps1")).Path) -Message "Release evidence session did not point to the evidence next-step launcher."
    ...
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Session State: evidence-in-progress")) -Message "Current evidence session summary did not record the expected session state."
    ...
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Evidence Ready Count: 0/3")) -Message "Current evidence session summary did not record the initial ready count."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Evidence Checklist Progress: $initialChecklistSummary")) -Message "Current evidence session summary did not record the initial checklist progress."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Evidence Progress: $($initialLoginAudit.ChecklistSummary)")) -Message "Current evidence session summary did not record the next evidence progress."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Evidence Tester: pending")) -Message "Current evidence session summary did not record the next evidence tester."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Current evidence session summary did not record the evidence action pack path."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Action Path: $initialNextEvidenceActionPath")) -Message "Current evidence session summary did not record the initial evidence next action path."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Current evidence session summary did not record the initial evidence next action launcher."

    $releaseExecutionPath = & $releaseValidationPassScript `
        -Mode stable `
        -VmType VirtualBox `
        -Firmware UEFI `
        -RunLabel $smokeRunLabel `
    ...
    $releaseExecutionEvidenceActionPackPath = Get-MetadataValue -Content $releaseExecutionContent -Label "Evidence Action Pack Path"
    $releaseExecutionEvidenceNextActionPath = Get-MetadataValue -Content $releaseExecutionContent -Label "Evidence Next Action Path"
    $releaseExecutionEvidenceNextActionLauncherPath = Get-MetadataValue -Content $releaseExecutionContent -Label "Evidence Next Action Launcher"
    Assert-Condition -Condition ((Resolve-Path $releaseExecutionEvidenceActionPackPath).Path -eq (Resolve-Path $releaseEvidenceActionPackPath).Path) -Message "Release validation pass did not bridge the evidence action pack path into execution metadata."
    Assert-Condition -Condition ((Resolve-Path $releaseExecutionEvidenceNextActionPath).Path -eq (Resolve-Path $initialNextEvidenceActionPath).Path) -Message "Release validation pass did not bridge the initial evidence next action path into execution metadata."
    Assert-Condition -Condition ((Resolve-Path $releaseExecutionEvidenceNextActionLauncherPath).Path -eq (Resolve-Path $initialNextEvidenceLauncherPath).Path) -Message "Release validation pass did not bridge the initial evidence next action launcher into execution metadata."
    ...
    Assert-Condition -Condition ($releaseExecutionRunbookContent -match [regex]::Escape("- Evidence Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Release validation runbook did not record the evidence action pack path."
    Assert-Condition -Condition ($releaseExecutionRunbookContent -match [regex]::Escape("- Evidence Next Action Path: $initialNextEvidenceActionPath")) -Message "Release validation runbook did not record the evidence next action path."
    Assert-Condition -Condition ($releaseExecutionRunbookContent -match [regex]::Escape("- Evidence Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Release validation runbook did not record the evidence next action launcher."
    ...
    Assert-Condition -Condition ($releaseExecutionWorkboardContent -match [regex]::Escape("- Evidence Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Release validation workboard did not record the evidence action pack path."
    Assert-Condition -Condition ($releaseExecutionWorkboardContent -match [regex]::Escape("- Evidence Next Action Path: $initialNextEvidenceActionPath")) -Message "Release validation workboard did not record the evidence next action path."
    Assert-Condition -Condition ($releaseExecutionWorkboardContent -match [regex]::Escape("- Evidence Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Release validation workboard did not record the evidence next action launcher."
    ...
    Assert-Condition -Condition ($releaseExecutionActionPackContent -match [regex]::Escape("- Evidence Action Pack: $releaseEvidenceActionPackPath")) -Message "Release validation action pack did not record the evidence action pack path."
    Assert-Condition -Condition ($releaseExecutionActionPackContent -match [regex]::Escape("- Evidence Next Action: $initialNextEvidenceActionPath")) -Message "Release validation action pack did not record the evidence next action path."
    Assert-Condition -Condition ($releaseExecutionActionPackContent -match [regex]::Escape("- Evidence Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Release validation action pack did not record the evidence next action launcher."
    Assert-Condition -Condition (Test-Path (Join-Path $releaseExecutionActionPackDir "00-run-next-step.ps1")) -Message "Release validation action pack did not create the next-step launcher."
    ...
    Assert-Condition -Condition (Test-Path (Join-Path $releaseExecutionActionPackDir "24-open-evidence-action-pack.ps1")) -Message "Release validation action pack did not create the evidence-action-pack helper."
    Assert-Condition -Condition (Test-Path (Join-Path $releaseExecutionActionPackDir "25-run-evidence-next-step.ps1")) -Message "Release validation action pack did not create the evidence next-step helper."
    ...
    $initialNextReleaseActionPath = & $openNextReleaseActionScript `
        -ExecutionPath $releaseExecutionPath `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    Assert-Condition -Condition ((Resolve-Path $initialNextReleaseActionPath).Path -eq (Resolve-Path (Join-Path $releaseExecutionActionPackDir "20-open-next-evidence.ps1")).Path) -Message "Next release action did not resolve to the next-evidence helper before evidence completion."
    ...
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Release validation sync did not keep the evidence action pack path."
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Next Action Path: $initialNextEvidenceActionPath")) -Message "Release validation sync did not keep the initial evidence next action path."
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Release validation sync did not keep the initial evidence next action launcher."
    ...
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Evidence Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Current release execution summary did not record the evidence action pack path."
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Evidence Next Action Path: $initialNextEvidenceActionPath")) -Message "Current release execution summary did not record the initial evidence next action path."
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Evidence Next Action Launcher: $initialNextEvidenceLauncherPath")) -Message "Current release execution summary did not record the initial evidence next action launcher."
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Next Action Path: $initialNextReleaseActionPath")) -Message "Current release execution summary did not record the initial next action path."
    ...
    $completedLoginAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseLoginTestReportPath -Target "login-test" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $completedInstallAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseInstallReportPath -Target "install" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $completedHardwareAudit = ((& $releaseEvidenceTargetAuditScript -ReportPath $releaseHardwareReportPath -Target "hardware" -AsJson) | Out-String).Trim() | ConvertFrom-Json
    $completedChecklistTotal = [int]$completedLoginAudit.TotalChecklistItems + [int]$completedInstallAudit.TotalChecklistItems + [int]$completedHardwareAudit.TotalChecklistItems
    $completedChecklistSummary = "$completedChecklistTotal/$completedChecklistTotal complete (100%)"

    Assert-Condition -Condition ($completedLoginAudit.ProgressState -eq "completed") -Message "Completed login-test audit did not report completed progress."
    Assert-Condition -Condition ($completedInstallAudit.ProgressState -eq "completed") -Message "Completed install audit did not report completed progress."
    Assert-Condition -Condition ($completedHardwareAudit.ProgressState -eq "completed") -Message "Completed hardware audit did not report completed progress."
    $currentEvidencePackContent = Get-Content -Raw $currentEvidencePackPath
    ...
    Assert-Condition -Condition ($currentEvidencePackContent -match [regex]::Escape("- Evidence Ready Count: 3/3")) -Message "Current evidence pack summary did not record the completed evidence ready count."
    Assert-Condition -Condition ($currentEvidencePackContent -match [regex]::Escape("- Evidence Checklist Progress: $completedChecklistSummary")) -Message "Current evidence pack summary did not record the completed checklist progress."
    $syncedReleaseEvidenceSessionPath = & $syncReleaseEvidenceSessionScript `
        -EvidenceSessionPath $releaseEvidenceSessionPath `
        -ReleaseVersion "0.1.0-ci" `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    Assert-Condition -Condition ((Resolve-Path $syncedReleaseEvidenceSessionPath).Path -eq (Resolve-Path $releaseEvidenceSessionPath).Path) -Message "Release evidence session sync did not return the original session path."
    $releaseEvidenceSessionContent = Get-Content -Raw $releaseEvidenceSessionPath
    Assert-Condition -Condition ($releaseEvidenceSessionContent -match [regex]::Escape("- Session State: ready-for-rc-gating")) -Message "Release evidence session did not advance to ready-for-rc-gating."
    ...
    $completedNextEvidenceActionPath = Get-MetadataValue -Content $releaseEvidenceSessionContent -Label "Next Action Path"
    $completedNextEvidenceLauncherPath = Get-MetadataValue -Content $releaseEvidenceSessionContent -Label "Next Action Launcher"
    Assert-Condition -Condition ((Resolve-Path $completedNextEvidenceActionPath).Path -eq (Resolve-Path $releaseEvidenceRunbookPath).Path) -Message "Release evidence session did not resolve the completed next action to the evidence runbook."
    Assert-Condition -Condition ((Resolve-Path $completedNextEvidenceLauncherPath).Path -eq (Resolve-Path (Join-Path $releaseEvidenceActionPackDir "00-run-next-step.ps1")).Path) -Message "Release evidence session did not keep the evidence next-step launcher after completion."
    $resolvedNextEvidencePath = & $openNextReleaseEvidenceScript `
        -EvidenceSessionPath $releaseEvidenceSessionPath `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    ...
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Evidence Ready Count: 3/3")) -Message "Current evidence session summary did not record the completed evidence ready count."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Evidence Checklist Progress: $completedChecklistSummary")) -Message "Current evidence session summary did not record the completed checklist progress."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Current evidence session summary did not keep the evidence action pack path."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Action Path: $completedNextEvidenceActionPath")) -Message "Current evidence session summary did not update the evidence next action path after completion."
    Assert-Condition -Condition ($currentEvidenceSessionContent -match [regex]::Escape("- Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Current evidence session summary did not keep the evidence next action launcher after completion."
    $currentReleaseControlCenterPath = Join-Path $tempRoot "status\\releases\\CURRENT-RELEASE-CONTROL-CENTER.md"
    ...
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Evidence Ready Count: 3/3")) -Message "Release control center did not record the completed evidence ready count."
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Evidence Checklist Progress: $completedChecklistSummary")) -Message "Release control center did not record the completed checklist progress."
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Evidence Action Pack: $releaseEvidenceActionPackPath")) -Message "Release control center did not record the evidence action pack path."
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Evidence Next Action: $completedNextEvidenceActionPath")) -Message "Release control center did not record the evidence next action path."
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Release control center did not record the evidence next action launcher."
    ...
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Action Pack Path: $releaseEvidenceActionPackPath")) -Message "Release validation pass did not keep the evidence action pack path after evidence completion."
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Next Action Path: $completedNextEvidenceActionPath")) -Message "Release validation pass did not update the evidence next action path after evidence completion."
    Assert-Condition -Condition ($releaseExecutionContent -match [regex]::Escape("- Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Release validation pass did not keep the evidence next action launcher after evidence completion."
    $postEvidenceNextReleaseActionPath = & $openNextReleaseActionScript `
        -ExecutionPath $releaseExecutionPath `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    Assert-Condition -Condition ((Resolve-Path $postEvidenceNextReleaseActionPath).Path -eq (Resolve-Path (Join-Path $releaseExecutionActionPackDir "50-audit-release-evidence.ps1")).Path) -Message "Next release action did not resolve to the evidence-audit helper after evidence completion."
    $currentReleaseExecutionContent = Get-Content -Raw $currentReleaseExecutionPath
    ...
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Evidence Next Action Path: $completedNextEvidenceActionPath")) -Message "Current release execution summary did not update the evidence next action path after evidence completion."
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Current release execution summary did not keep the evidence next action launcher after evidence completion."
    Assert-Condition -Condition ($currentReleaseExecutionContent -match [regex]::Escape("- Next Action Path: $postEvidenceNextReleaseActionPath")) -Message "Current release execution summary did not update the next action path after evidence completion."
    ...
    $postAuditNextReleaseActionPath = & $openNextReleaseActionScript `
        -ExecutionPath $releaseExecutionPath `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    Assert-Condition -Condition ((Resolve-Path $postAuditNextReleaseActionPath).Path -eq (Resolve-Path (Join-Path $releaseExecutionActionPackDir "60-prepare-release-candidate.ps1")).Path) -Message "Next release action did not resolve to the RC helper after evidence audit."
    ...
    $currentReleaseControlCenterContent = Get-Content -Raw $currentReleaseControlCenterPath
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Release Control State: ready-for-readiness-audit")) -Message "Release control center did not advance to readiness-audit state."
    ...
    $currentReleaseControlCenterContent = Get-Content -Raw $currentReleaseControlCenterPath
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Release Control State: ready-to-publish")) -Message "Release control center did not advance to ready-to-publish."
    ...
    $currentReleaseControlCenterContent = Get-Content -Raw $currentReleaseControlCenterPath
    Assert-Condition -Condition ($currentReleaseControlCenterContent -match [regex]::Escape("- Release Control State: published")) -Message "Release control center did not switch to published."
...
"@ -Encoding UTF8
    ...
    $finalNextReleaseActionPath = & $openNextReleaseActionScript `
        -ExecutionPath $releaseExecutionPath `
        -RepoRoot $tempRoot `
        -OutputPathOnly
    ...
    Assert-Condition -Condition ($shareableContent -match [regex]::Escape("- Release Next Action: $finalNextReleaseActionPath")) -Message "Shareable update did not include the resolved next release action."
    Assert-Condition -Condition ($shareableContent -match [regex]::Escape("- Release Evidence Action Pack: $releaseEvidenceActionPackPath")) -Message "Shareable update did not include the evidence action pack path."
    Assert-Condition -Condition ($shareableContent -match [regex]::Escape("- Release Evidence Next Action: $completedNextEvidenceActionPath")) -Message "Shareable update did not include the evidence next action path."
    Assert-Condition -Condition ($shareableContent -match [regex]::Escape("- Release Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Shareable update did not include the evidence next action launcher."
    ...
    Assert-Condition -Condition ($shareableContent -match [regex]::Escape("- Release Control State: published")) -Message "Shareable update did not include the release-control state."
    ...
    Assert-Condition -Condition ($shareableBriefContent -match [regex]::Escape("- Release Next Action: $finalNextReleaseActionPath")) -Message "English shareable brief did not include the resolved next release action."
    Assert-Condition -Condition ($shareableBriefContent -match [regex]::Escape("- Release Evidence Action Pack: $releaseEvidenceActionPackPath")) -Message "English shareable brief did not include the evidence action pack path."
    Assert-Condition -Condition ($shareableBriefContent -match [regex]::Escape("- Release Evidence Next Action: $completedNextEvidenceActionPath")) -Message "English shareable brief did not include the evidence next action path."
    Assert-Condition -Condition ($shareableBriefContent -match [regex]::Escape("- Release Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "English shareable brief did not include the evidence next action launcher."
    ...
    Assert-Condition -Condition ($shareableBriefContent -match [regex]::Escape("- Release Control State: published")) -Message "English shareable brief did not include the release-control state."
    ...
    Assert-Condition -Condition ($shareableArabicBriefContent -match [regex]::Escape("- Release Next Action: $finalNextReleaseActionPath")) -Message "Arabic shareable brief did not include the resolved next release action."
    Assert-Condition -Condition ($shareableArabicBriefContent -match [regex]::Escape("- Release Evidence Action Pack: $releaseEvidenceActionPackPath")) -Message "Arabic shareable brief did not include the evidence action pack path."
    Assert-Condition -Condition ($shareableArabicBriefContent -match [regex]::Escape("- Release Evidence Next Action: $completedNextEvidenceActionPath")) -Message "Arabic shareable brief did not include the evidence next action path."
    Assert-Condition -Condition ($shareableArabicBriefContent -match [regex]::Escape("- Release Evidence Next Action Launcher: $completedNextEvidenceLauncherPath")) -Message "Arabic shareable brief did not include the evidence next action launcher."
    ...
    Assert-Condition -Condition ($shareableArabicBriefContent -match [regex]::Escape("- Release Control State: published")) -Message "Arabic shareable brief did not include the release-control state."
